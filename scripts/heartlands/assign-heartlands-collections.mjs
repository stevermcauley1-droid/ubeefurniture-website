#!/usr/bin/env node
/**
 * Assign Heartlands (vendor) products to storefront collections.
 *
 * Store smart collections (see scripts/create-collections.ts) match on productType OR tag.
 * Imported Heartlands SKUs used productType "Furniture", so they never matched — we fix via productUpdate.
 *
 * When a target smart collection exists, we set productType + add the matching tag (OR rules).
 * Unmatched titles go to a manual fallback collection (handle: supplier-heartlands-catalog) via collectionAddProducts.
 *
 * Env: SHOPIFY_STORE_DOMAIN, SHOPIFY_ADMIN_API_TOKEN or SHOPIFY_ADMIN_ACCESS_TOKEN
 *
 * Usage:
 *   node scripts/heartlands/assign-heartlands-collections.mjs
 *   node scripts/heartlands/assign-heartlands-collections.mjs --dry-run
 */
...
const FALLBACK_MANUAL_HANDLE = "supplier-heartlands-catalog";
const FALLBACK_MANUAL_TITLE = "Heartlands catalog";
...
function classifyTitle(title) {
  ...
  if (/\bsofa\b|\bsettee\b|\bcouch\b/.test(t)) {
...
async function fetchAllHeartlandsProducts(domain, token) {
  const out = [];
  let cursor = null;
  let hasNext = true;
  while (hasNext) {
    const data = await adminGraphql(
      domain,
      token,
      `query HeartlandsProducts($cursor: String) {
        products(first: 50, after: $cursor, query: "vendor:Heartlands", sortKey: UPDATED_AT) {
...
async function ensureManualFallbackCollection(domain, token, publicationIds) {
  ...
  const created = await adminGraphql(
    ...
    }`,
    {
      input: {
        ...
        descriptionHtml: "<p>Heartlands products pending manual categorization.</p>",
      },
    }
  );
  ...
  try {
    ...
    console.warn("[heartlands-collections] publish new manual collection:", e.message);
  }
  return id;
}
...
async function collectionAddProductsWithRetry(
  ...
  for (let i = 0; i < attempts; i++) {
    try {
      ...
      // Retry transient API/server failures only.
...
/** Duplicate / already-member cases (wording varies by API version). */
function isSkippableCollectionAddError(message) {
  ...
  // Observed: "Error adding gid://shopify/Product/… to collection"
...
async function main() {
  ...
  console.error("[heartlands-collections] Loading collections index…");
  ...
  if (missing.length) {
    console.warn(
      "[heartlands-collections] Missing collection handles (create in Admin or run scripts/create-collections.ts):",
      missing.join(", ")
    );
  }

  console.error("[heartlands-collections] Loading vendor:Heartlands products…");
  const products = await fetchAllHeartlandsProducts(domain, token);
  if (!products.length) {
    console.error("No products with vendor Heartlands.");
    process.exit(1);
  }
  ...
  for (let i = 0; i < products.length; i++) {
    ...
    try {
      ...
      if (cls) {
        ...
        const mergedTags = [...new Set([...baseTags, cls.tag, "heartlands", "supplier-heartlands", "featured"])];
        ...
        const mergedTags = [...new Set([...baseTags, "featured", "heartlands", "supplier-heartlands"])];
        await productUpdate(domain, token, {
          id: p.id,
          tags: mergedTags,
  ...
  if (!dryRun && fallbackProductIds.length && manualFallbackId) {
    ...
    let manualDup = 0;
    let manualFail = 0;
    // Per-product: any duplicate in a chunk fails the whole chunk in Shopify; singles are idempotent-ish.
    for (let i = 0; i < unique.length; i++) {
      ...
      try {
        ...
        if (isSkippableCollectionAddError(msg)) {
          ...
          manualFail++;
          console.warn(`[heartlands-collections] manual add ${pid}:`, msg);
        }
      }
    ...
    manualAdds = manualOk;
    console.error(
      `[heartlands-collections] Manual ${FALLBACK_MANUAL_HANDLE}: added=${manualOk} already_in=${manualDup} other_errors=${manualFail}`
    );
