#!/usr/bin/env node
/**
 * Assign Beehive Furniture Wholesale (vendor) products to storefront collections.
 *
 * Same rules as Heartlands: set productType + tag so smart collections match; unmatched → manual fallback.
 *
 * Env: SHOPIFY_STORE_DOMAIN, SHOPIFY_ADMIN_API_TOKEN or SHOPIFY_ADMIN_ACCESS_TOKEN
 *
 * Usage:
 *   node scripts/beehive/assign-beehive-collections.mjs
 *   node scripts/beehive/assign-beehive-collections.mjs --dry-run
 */
...
/** Exact vendor string from Beehive portal / import JSON. */
const BEEHIVE_VENDOR_QUERY = 'vendor:"Beehive Furniture Wholesale"';

const FALLBACK_MANUAL_HANDLE = "supplier-beehive-catalog";
const FALLBACK_MANUAL_TITLE = "Beehive catalog";
...
async function fetchAllBeehiveProducts(domain, token) {
  const out = [];
  let cursor = null;
  let hasNext = true;
  while (hasNext) {
    const data = await adminGraphql(
      domain,
      token,
      `query BeehiveProducts($cursor: String, $q: String!) {
...
async function ensureManualFallbackCollection(domain, token, publicationIds) {
  ...
  const created = await adminGraphql(
    ...
    }`,
    {
      input: {
        ...
        descriptionHtml: "<p>Beehive products pending manual categorization.</p>",
      },
    }
  );
  ...
  try {
    ...
    console.warn("[beehive-collections] publish new manual collection:", e.message);
  }
  return id;
}
...
async function main() {
  ...
  console.error("[beehive-collections] Loading collections index…");
  ...
  if (missing.length) {
    console.warn(
      "[beehive-collections] Missing collection handles (create in Admin or run scripts/create-collections.ts):",
      missing.join(", ")
    );
  }

  console.error(`[beehive-collections] Loading products (${BEEHIVE_VENDOR_QUERY})…`);
  const products = await fetchAllBeehiveProducts(domain, token);
  if (!products.length) {
    console.error("No Beehive vendor products found. Import first: npm run beehive:import");
    process.exit(1);
  }
  ...
  for (let i = 0; i < products.length; i++) {
    ...
    try {
      ...
      if (cls) {
        ...
        const mergedTags = [...new Set([...baseTags, cls.tag, "beehive", "supplier-beehive", "featured"])];
        ...
        const mergedTags = [...new Set([...baseTags, "featured", "beehive", "supplier-beehive"])];
        await productUpdate(domain, token, {
          id: p.id,
          tags: mergedTags,
  ...
  if (!dryRun && fallbackProductIds.length && manualFallbackId) {
    ...
    for (let i = 0; i < unique.length; i++) {
      ...
      try {
        ...
        if (isSkippableCollectionAddError(msg)) {
          ...
          manualFail++;
          console.warn(`[beehive-collections] manual add ${pid}:`, msg);
        }
      }
    ...
    manualAdds = manualOk;
    console.error(
      `[beehive-collections] Manual ${FALLBACK_MANUAL_HANDLE}: added=${manualOk} already_in=${manualDup} other_errors=${manualFail}`
    );
