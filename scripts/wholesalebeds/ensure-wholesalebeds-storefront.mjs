#!/usr/bin/env node
/**
 * Ensure Wholesale Beds vendor products are ACTIVE and published to
 * Online Store + Headless (runs assign-wholesalebeds-collections first).
 *
 * Env: SHOPIFY_STORE_DOMAIN, SHOPIFY_ADMIN_API_TOKEN or SHOPIFY_ADMIN_ACCESS_TOKEN
 *
 * Usage:
 *   node scripts/wholesalebeds/ensure-wholesalebeds-storefront.mjs
 *   node scripts/wholesalebeds/ensure-wholesalebeds-storefront.mjs --skip-assign
 */
...
async function fetchWholesaleBedsProductNodes(domain, token) {
  const ids = [];
  let cursor = null;
  let hasNext = true;
  while (hasNext) {
    const data = await adminGraphql(
      ...
      { c: cursor, q: WHOLESALE_BEDS_VENDOR_QUERY }
    );
...
async function main() {
  ...
  if (!skipAssign) {
    console.error("[wholesalebeds-storefront] Running assign-wholesalebeds-collections…");
    const r = spawnSync(process.execPath, ["scripts/wholesalebeds/assign-wholesalebeds-collections.mjs"], {
      cwd: process.cwd(),
      stdio: "inherit",
      env: process.env,
    });
    if (r.status !== 0) {
      console.error("[wholesalebeds-storefront] assign-wholesalebeds-collections exited", r.status);
      process.exit(r.status ?? 1);
    }
  }

  console.error("[wholesalebeds-storefront] Loading publications…");
  const targets = await getPublicationTargets(domain, token);
  console.error("[wholesalebeds-storefront] Publications:", targets.allNames.join(", "));
  const inputs = publicationInputs(targets);
  if (!inputs.length) {
    console.error("[wholesalebeds-storefront] No Online Store / Headless publication IDs found.");
    process.exit(1);
  }

  console.error("[wholesalebeds-storefront] Loading Wholesale Beds vendor products…");
  const nodes = await fetchWholesaleBedsProductNodes(domain, token);
  if (!nodes.length) {
    console.error("[wholesalebeds-storefront] No Wholesale Beds vendor products. Run npm run wholesalebeds:import first.");
    process.exit(1);
  }
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    try {
      ...
      console.error(`[wholesalebeds-storefront] ${n.title}:`, e.message || e);
    }
  }

  console.error(
    `[wholesalebeds-storefront] Products: total=${nodes.length} activated=${activated} channel_publish=${published} already_ok≈${skipped}`
  );
  console.error("\n[wholesalebeds-storefront] Done.");
}
